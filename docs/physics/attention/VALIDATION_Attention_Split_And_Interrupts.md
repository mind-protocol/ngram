...
ONLY CanonHolder may:
  - transition moment.status -> spoken
  - write THEN links
  - emit moment_spoken SSE
```
...
Interrupt = YES iff one of:
  ...
  (c) a moment becomes spoken in player neighborhood
...
If not linked, it can exist but never becomes spoken for the player stream.
```
...
Within a beat window:
  at most one candidate is spoken
