...
```
@ngram:id: HEALTH.PHYSICS.ENERGY.V1.2
STATUS: DRAFT
CREATED: 2024-12-23
```
...
VALIDATION:      ./VALIDATION_Energy_Physics.md
IMPLEMENTATION:  ./IMPLEMENTATION_Energy_Physics.md
THIS:            HEALTH_Energy_Physics.md (you are here)
SYNC:            ./SYNC_Physics.md

IMPL:            engine/physics/health/checker.py
```
...
flows_analysis:
  - flow_id: tick_execution
    ...
    triggers:
      - type: schedule
        source: engine/infrastructure/orchestration/world_runner.py:tick
  ...
  - flow_id: energy_generation
    ...
    triggers:
      - type: event
        source: engine/physics/tick.py:phase_generate
  ...
  - flow_id: moment_draw
    ...
    triggers:
      - type: event
        source: engine/physics/tick.py:phase_draw
  ...
  - flow_id: moment_flow
    ...
    triggers:
      - type: event
        source: engine/physics/tick.py:phase_flow
  ...
  - flow_id: link_traversal
    ...
    triggers:
      - type: event
        source: engine/physics/flow.py:energy_flows_through
  ...
  - flow_id: link_cooling
    ...
    triggers:
      - type: event
        source: engine/physics/tick.py:phase_link_cooling
  ...
  - flow_id: moment_lifecycle
    ...
    triggers:
      - type: event
        source: engine/infrastructure/canon/canon_holder.py:transition
  ...
  - flow_id: top_n_filter
    ...
    triggers:
      - type: event
        source: engine/physics/tick.py:get_hot_links
...
```yaml
status:
  stream_destination: engine/physics/health/status.json
  result:
    representation: enum
    value: UNKNOWN
...
docks:
  input:
    ...
    location: engine/physics/tick.py:99
  output:
    id: DOCK-TICK-END
    method: engine.physics.tick.snapshot_energy_state
    location: engine/physics/tick.py:145
```
...
forwarding:
  targets:
    - location: engine/physics/health/status.json
      transport: file
...
docks:
  input:
    ...
    location: engine/physics/graph/graph_queries.py:88
  output:
    id: DOCK-ALL-LINK-ENERGIES
    method: engine.physics.graph.get_all_link_energies
    location: engine/physics/graph/graph_queries.py:95
```
...
docks:
  ...
  output:
    ...
    location: engine/physics/tick.py:get_hot_links
```
...
docks:
  input:
    ...
    location: engine/physics/tick.py:_record_phase
  output:
    id: DOCK-TICK-END
...
docks:
  input:
    ...
    location: engine/models/nodes.py:Moment
  output:
    id: DOCK-MOMENT-TRANSITION
    method: engine.infrastructure.canon.canon_holder.transition
    location: engine/infrastructure/canon/canon_holder.py:78
```
...
docks:
  input:
    ...
    location: engine/physics/flow.py:15
  output:
    id: DOCK-TRAVERSAL
    method: engine.physics.flow.energy_flows_through
    location: engine/physics/flow.py:25
```
...
<!-- @ngram:todo Missing checker for V-GEN-PROXIMITY-BOUNDS (proximity_bounds indicator) -->
<!-- @ngram:todo Missing checker for V-GEN-PROXIMITY-CORRELATION (proximity correlation) -->
<!-- @ngram:todo Missing checker for V-TOPN-CRITICAL (top_n_coverage indicator) -->
<!-- @ngram:todo Missing checker for V-TOPN-DETERMINISTIC (filter determinism) -->
<!-- @ngram:todo Missing checker for V-EMOTION-BASELINE (emotion blending correctness) -->
<!-- @ngram:todo Missing checker for V-EMOTION-BLEND (emotion intensity bounds) -->
<!-- @ngram:todo Missing checker for V-CRYSTAL-ON-COMPLETE (crystallization link creation) -->
<!-- @ngram:todo Missing checker for V-BACKFLOW-GATED (backflow only to focused) -->
<!-- @ngram:todo Missing checker for V-MOMENT-DRAW-BOUNDS (draw doesn't exceed available) -->
<!-- @ngram:todo Missing checker for V-MOMENT-DURATION-RATE (radiation rate matches duration) -->
<!-- @ngram:todo No integration test for full tick under load -->
<!-- @ngram:todo Docking points not yet instrumented in tick.py -->
<!-- @ngram:todo Health checker CLI not yet implemented -->
...
| @ngram:id | Category | Priority | Checker Status |
|-----------|----------|----------|----------------|
...
<!-- @ngram:todo Implement energy_conservation_checker (maps to V-ENERGY-BOUNDED, V-ENERGY-CONSERVED) -->
<!-- @ngram:todo Implement no_negative_checker (maps to V-ENERGY-NON-NEGATIVE) -->
<!-- @ngram:todo Implement link_state_checker (maps to V-LINK-ALIVE, V-LINK-BOUNDED) -->
<!-- @ngram:todo Implement tick_integrity_checker (maps to V-TICK-ORDER, V-TICK-COMPLETE) -->
<!-- @ngram:todo Implement moment_lifecycle_checker (maps to V-MOMENT-TRANSITIONS) -->
<!-- @ngram:todo Implement strength_growth_checker (maps to V-LINK-STRENGTH-GROWTH) -->
<!-- @ngram:todo Implement proximity_checker (maps to V-GEN-PROXIMITY-BOUNDS) -->
<!-- @ngram:todo Implement top_n_checker (maps to V-TOPN-CRITICAL) -->
<!-- @ngram:todo Add docking point instrumentation to tick.py -->
<!-- @ngram:todo Create health checker CLI entry point -->
<!-- @ngram:todo Set up status.json output format -->
