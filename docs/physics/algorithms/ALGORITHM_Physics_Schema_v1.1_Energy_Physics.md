...
```
CREATED: 2025-12-23
STATUS: CANONICAL (supersedes previous energy flow logic)
```
...
THIS:           ALGORITHM_Physics_Schema_v1.1_Energy_Physics.md
...
Moments FLOW to all connected nodes (narratives, actors, spaces)
    ↓ via conductivity × weight × emotion_proximity
Completion: LIQUIDATE to connected nodes, become inert bridge
```

### The Unified Formula

**All energy flow uses one formula:**

```
flow = source.energy × base_rate × link.conductivity × link.weight × emotion_factor
```
...
for moment in active_moments:
    ...
    for link in actor_links_to_moment:
        ...
        flow_rate = link.conductivity × link.weight × emotion_factor
        ...
        link.energy += draw
        link.strength += draw × 0.1

        # Hebbian: color link with moment's emotions
        link.emotions = blend(link.emotions, weighted_emotions, draw/(draw+1))
```

**Why speaker draws more:** Their link has high conductivity (1.0), high weight (1.0), matching emotions. No special case needed.

**Why witness draws less:** Lower conductivity, lower weight, partial emotion match.
...
for moment in active_moments:
    ...
    for link in outgoing_links:
        ...
        # Base flow
        emotion_factor = emotion_proximity(link.emotions, weighted_emotions)
        base_flow = moment.energy × flow_rate × link.conductivity × link.weight × emotion_factor
...
for narrative in narratives:
    ...
    for link in narrative.actor_links:
        ...
        flow = narrative.energy × backflow_rate × link.conductivity × emotion_factor
        actor.energy += flow
```
...
def path_resistance(from_id, to_id):
    """Dijkstra with resistance = 1/(conductivity × weight × emotion_factor)"""

    # Each edge: low conductivity = high resistance
    edge_resistance = 1 / (link.conductivity × link.weight × emotion_factor)
...
**High conductivity path:** resistance low → flow factor high → energy flows easily

**Low conductivity path:** resistance high → flow factor low → energy blocked
...
LinkBase:
  fields:
    ...
    conductivity: float [0,1], default 1.0
    weight: float [0,∞], default 1.0
    energy: float [0,∞], default 0  # current attention
    strength: float [0,∞], default 0  # accumulated depth
...
<!-- @ngram:todo Implement all 6 tick phases in engine/physics/tick.py -->
<!-- @ngram:todo Add path resistance Dijkstra to graph_ops -->
<!-- @ngram:todo Implement link crystallization on moment completion -->
<!-- @ngram:todo Add moment state machine (possible→active→completed) to canon_holder -->
<!-- @ngram:todo Implement narrative backflow in physics tick -->
...
priority: 4
context: "When actors share a moment, crystallization creates relates link. Current spec: conductivity=0.2, weight=0.2"
options:
