...
|-------------|----------|-----------------------------|
| B1 | The bundle splitter writes every `### path` section to its documented destination file and replaces each `$$$` fence with standard Markdown fences. | Guarantees that doc exports do not vanish or leave stray delimiters, so downstream editors and `ngram doctor` can reproduce the converted assets deterministically. |
...
SYMPTOM: downstream `ngram doctor` alerts about missing fixtures.
```

## HEALTH COVERAGE

- `tools/HEALTH_Tools.md` instructs operators to run `ngram doctor` so the whole `docs/tools/` chain stays indexed and validated.
