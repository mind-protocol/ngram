...
### DocExtractor (ngram/doc_extractor.py)
...
class DocExtractor:
    ...
    def _extract_markers(self, content: str) -> List[Marker]:
        """Find @ngram:todo, @ngram:escalation, etc."""
        ...
...
### ClusterBuilder (ngram/cluster_builder.py)
...
@dataclass
class Marker:
    """Extracted marker (@ngram:todo, etc.)."""
...
|-------------|-------------|
| `ngram doctor --cluster` | Extract docs during doctor scan |
| `ngram cluster --doc X` | Extract single document |
| `ngram cluster --all` | Extract all documents |
...
- ngram/symbol_extractor.py — Code symbol extraction (similar pattern)
