...
def extract_markers(content):
    """Extract @ngram:type markers."""
    patterns = {
        'todo': r'@ngram:todo\s+(.+)',
        'escalation': r'@ngram:escalation\s+(.+)',
        'proposition': r'@ngram:proposition\s+(.+)',
    }
    markers = []
...
docks:
  - name: dock_graph_connection
    direction: input
    uri: "engine/graph/health/check_health.py::GraphOps._query"
    line: 272
...
checkers:
  - name: check_health_cli
    status: active
    priority: high
    implemented_by: "engine/graph/health/check_health.py"
...
<!-- @ngram:todo Add physics range checks (V2) -->
```
