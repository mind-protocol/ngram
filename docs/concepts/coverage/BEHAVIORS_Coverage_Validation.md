...
@ngram:id: BEHAVIORS.COVERAGE.VALIDATION.SYSTEM
...
✓ D-UNDOC-CODE → ngram.create_module_docs
✓ D-PLACEHOLDER-DOCS → ngram.create_module_docs
✗ D-ORPHAN-DOCS → ngram.unknown_skill (MISSING)
```
...
✓ ngram.create_module_docs → [explore_space, create_doc_chain]
✗ ngram.create_module_docs → create_doc_chain (PROTOCOL MISSING)
```
...
GAP: Protocol 'create_doc_chain' missing
  Referenced by: skill ngram.create_module_docs
