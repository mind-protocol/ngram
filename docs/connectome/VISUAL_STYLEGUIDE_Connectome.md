...
3. The Membrane is Invisible
   The Membrane is a core architectural component: a pre-runtime field shaper, not a visual effect. It is the system's modulation layer, influencing the graph by adjusting mechanical parameters (weight_transfer, thresholds, tension_boosts) before the hot path execution begins. Visually, its influence must be rendered as an atmospheric or environmental quality, never as an explicit UI control. The Membrane "tilts the ground; it does not move the pieces." Its effect is perceived as a change in the system's gravity, not as a visible hand guiding its operations.
