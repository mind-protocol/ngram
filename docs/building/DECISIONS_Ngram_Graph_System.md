# DECISIONS: Ngram Graph System
@ngram:id: DECISIONS.NGRAM.GRAPH.SYSTEM
...
This document resolves escalation markers from the Ngram Graph System doc chain.
...
2. Second failure: Mark moment as `rejected`, return energy to source
...
- `ngram query` CLI for structured queries
...
- `ngram ingest --changed` processes queue
- Full re-ingest via `ngram ingest --full`
...
4. **Remove escalations:** Once DECIDED, remove corresponding `@ngram:escalation` marker

---

## CHAIN

- **Related:** PATTERNS_Ngram_Graph_System.md, ALGORITHM_Ngram_Graph_System.md
