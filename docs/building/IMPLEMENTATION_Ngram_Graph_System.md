# ngram Graph System — Implementation: Code Architecture and Structure
...
THIS:            IMPLEMENTATION_Ngram_Graph_System.md (you are here)
...
**We are a client of the ngram engine.**
...
│   ├── markers.py           # @ngram: marker extraction
...
| `ingest/markers.py` | Extract @ngram markers | `extract_markers()` | PLANNED |
...
flow:
  ...
  steps:
    ...
    - id: extract_markers
      description: Find @ngram markers
...
| VALIDATION V-NGRAM-INGEST-* | `tests/building/test_ingest.py` |
| VALIDATION V-NGRAM-AGENT-* | `tests/building/test_agents.py` |

---

## OPEN QUESTIONS

<!-- @ngram:escalation How do we register on_agent_trigger with engine? Callback registration API? -->

<!-- @ngram:escalation What's the engine.get_context() signature? Do we pass thresholds or engine owns them? -->

<!-- @ngram:escalation How do we handle agent failure? Retry? Mark moment as failed? -->

<!-- @ngram:escalation Response parsing: structured output from LLM? Or parse freeform text? -->

<!-- @ngram:proposition Consider making ingest incremental (watch for file changes) -->
...
<!-- @ngram:todo Implement ingest/discover.py -->
<!-- @ngram:todo Implement ingest/parse.py -->
<!-- @ngram:todo Implement agents/handler.py -->
<!-- @ngram:todo Define agents.yaml schema -->
<!-- @ngram:todo Write tests for type inference -->
