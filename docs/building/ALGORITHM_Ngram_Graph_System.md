# ngram Graph System — Algorithm: Client Procedures
...
THIS:            ALGORITHM_Ngram_Graph_System.md (you are here)
...
FOR each agent in agents.yaml:
  ...
  engine.create_link(type: contains, from: agent.space, to: actor_id)
```
...
```
opening = f"Active moment: {moment.prose}"
...
| engine.create_link() | Link with conductivity |
...
<!-- @ngram:todo Define NARRATIVE_WEIGHTS, NARRATIVE_ENERGIES constants -->
<!-- @ngram:todo Implement incremental ingest for file changes -->
<!-- @ngram:todo Design agent base prompt loading -->
<!-- @ngram:todo Implement response parsing for moments/narratives -->

<!-- @ngram:escalation Type inference is heuristic-based — will misclassify. Should agents be able to correct/reclassify? -->
<!-- @ngram:escalation Ingest runs once at bootstrap — what about docs created after? File watcher? Git hook? Manual re-ingest? -->
<!-- @ngram:escalation Agent handler calls LLM synchronously — what about timeout? Retry? Partial response? -->
