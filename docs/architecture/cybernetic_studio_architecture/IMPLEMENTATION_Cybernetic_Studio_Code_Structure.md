...
```
ngram/                      # Studio Platform (graph physics, ingestion, orchestration)
├── ngram/                  # CLI, orchestration, health/validation tooling
├── docs/                   # Protocol + module docs
└── data/                   # Architecture sources (conceptual)

blood-ledger/               # Game Cartridge (world content + player experience)
├── engine/                 # Runtime orchestration, physics integration
├── frontend/               # Player UI
...
flow:
  ...
  steps:
    ...
    - id: derive_l2
      ...
      file: ngram (stimulus derivation, planned)
