...
This algorithm describes the system-level flow from stimuli in the repos to surfaced, canonized knowledge in the shared graph. It is conceptual and applies across `ngram` and `blood-ledger` with the graph as the physics layer.
...
|--------|--------------|-------------|
| `ngram` | stimulus ingestion | L1/L2 stimuli |
