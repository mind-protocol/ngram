...
This document defines the invariants and properties that all Pydantic data models within the `engine/models/` module must uphold. These validations ensure the structural integrity, type consistency, and logical correctness of the data before it is persisted in the graph or used by other engine components.
...
```
[ ] Review all model definitions in `engine/models/` for correct type hints, `Field` constraints, and enum usage.
...
pytest tests/engine/models/ -v

# Run tests specifically for base models and enums
pytest tests/engine/models/test_base_models.py -v
```
