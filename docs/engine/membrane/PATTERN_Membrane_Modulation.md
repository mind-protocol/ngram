...
|--------|---------------------|
| `engine/physics/tick.py` | Applies per-tick thresholds and decay rates |
| `engine/moment_graph/*` | Consumes transfer coefficients and traversal thresholds |
...
| `docs/engine/moments/PATTERNS_Moments.md` | Player-visible artifact constraints |
...
<!-- @ngram:todo Define the Modulation Frame schema and bounds. -->
<!-- @ngram:todo Decide where modulation is applied in tick order (pre- or post-decay). -->
<!-- @ngram:todo Add a health check for over/under-modulation. -->
<!-- @ngram:todo Document membrane momentum state and decay rules. -->
