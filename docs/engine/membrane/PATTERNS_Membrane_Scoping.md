...
IMPL:            engine/moment_graph/queries.py
IMPL:            engine/moment_graph/surface.py
IMPL:            engine/membrane/functions.py
...
3. For dynamic modulation function updates, align `engine/membrane/functions.py` and `engine/physics/tick.py`, or add a TODO in `SYNC_Membrane_Modulation.md` describing the needed hook.
4. Run `ngram validate`.
...
|--------|---------------------|
| `engine/moment_graph/queries.py` | Provides scene scope and local aggregates |
| `engine/moment_graph/surface.py` | Scene changes and surfacing context |
| `engine/membrane/functions.py` | Hosts the dynamic parameter functions that replace magic constants |
| `engine/physics/tick.py` | Applies the modulation frame during traversal and uses the dynamic thresholds/decays |
| `docs/engine/moment-graph-engine/PATTERNS_Instant_Traversal_Moment_Graph.md` | Hot path constraints |
