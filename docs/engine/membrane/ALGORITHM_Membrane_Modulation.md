...
|--------|--------------|-------------|
| `engine/moment_graph/queries.py` | `get_current_view` | aggregate metrics |
| `engine/membrane/functions.py` | `activation_threshold` | dynamic thresholds |
| `engine/membrane/provider.py` | `set_frame` | cached frame by place_id |

---

## MARKERS

<!-- @ngram:todo Define aggregate metrics used for v0. -->
<!-- @ngram:todo Document default bounds for each modifier. -->
