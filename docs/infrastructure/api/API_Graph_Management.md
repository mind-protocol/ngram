...
```
DOCS: engine/infrastructure/api/graphs.py
STATUS: IMPLEMENTED
UPDATED: 2025-12-22
```
...
Generic graph lifecycle endpoints for ngram consumers. Games (like blood-ledger) call these to create isolated graph instances for their sessions.

**Ownership boundary:**
- ngram owns: graph creation, cloning, deletion, generic queries
...
|------------------|--------------|-------|
| `engine/infrastructure/api/playthroughs.py` | blood-ledger | All playthrough logic |
...
All endpoints implemented in `engine/infrastructure/api/graphs.py`:
