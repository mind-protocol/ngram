...
```
GIVEN:  templates exist inside the installed package and include the ngram subtree
...
```
GIVEN:  package templates do not exist, but repo-root templates include the ngram subtree
WHEN:   get_templates_path() is called
...
|--------|------|-------------|
| templates_path | Path | directory that contains templates/ngram |
...
### A2: Return templates path that lacks the ngram subtree

```
GIVEN:   a templates directory exists but is missing the ngram subfolder
WHEN:    get_templates_path() is called
