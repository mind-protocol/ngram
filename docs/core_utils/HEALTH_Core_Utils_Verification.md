...
value_and_validation:
  ...
  validation:
    - validation_id: V1
      criteria: get_templates_path returns a templates directory that includes templates/ngram
```
...
docks:
  output:
    ...
    method: ngram.core_utils.get_templates_path
    location: ngram/core_utils.py:36
```
...
```yaml
mechanism:
  summary: call get_templates_path and confirm returned path includes templates/ngram
  steps:
    - call get_templates_path()
    - verify returned path contains the ngram subtree
  data_required: filesystem state
  failure_mode: FileNotFoundError or missing ngram subtree
```
...
docks:
  output:
    ...
    method: ngram.core_utils.find_module_directories
    location: ngram/core_utils.py:67
```
