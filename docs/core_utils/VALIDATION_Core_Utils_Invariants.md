...
### V1: Template path includes the ngram subtree

```
get_templates_path() must only return a templates directory that contains templates/ngram.
```

**Checked by:** manual verification of `get_templates_path` in `ngram/core_utils.py`
...
|-----------|--------|--------|
| V1: Template path includes ngram | Manual call to get_templates_path | ⚠ NOT YET VERIFIED |
...
VERIFIED_AGAINST:
    impl: ngram/core_utils.py @ working tree
    test: n/a
