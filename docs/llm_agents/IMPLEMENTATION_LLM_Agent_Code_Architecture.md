...
|------|---------|-----------------------|-------|--------|
| `ngram/llms/gemini_agent.py` | Launches the Gemini subprocess, configures the SDK, streams structured JSON, and shields CLI logic from provider SDKs. | `main`, the tool helper definitions, `tool_map`, streaming loop. | ~270 | OK |
...
The CLI (`ngram/agent_cli.py`) builds `python -m ngram.llms.gemini_agent` with the selected provider arguments. Because the entire adapter fits within `ngram.llms`, no extra modules exist yet, but the CLI and adapter remain tightly coupled by the shared command-line schema.
...
|---------|------------|---------|
| Strategy | `tool_map` inside `ngram/llms/gemini_agent.py` | Allows the Gemini response to invoke different helper functions by name without inlining call handling logic. |
...
|----------|--------|---------|-----------|
| CLI ↔ Gemini adapter | Argument parsing, credential validation, tool helper wiring | TUI, agent_cli routing, downstream tool runners | CLI wraps `python -m ngram.llms.gemini_agent` with flags and trusts the JSON stream. |
...
|-------------|-----------|--------------|
| `main()` | `ngram/llms/gemini_agent.py:14` | Called when `ngram/agent_cli.py` launches `python -m ngram.llms.gemini_agent` for the `gemini` provider. |
...
flow:
  ...
  steps:
    - id: cli_launch
      description: CLI builds `python -m ngram.llms.gemini_agent` with provider flags and environment-derived credentials.
      file: ngram/agent_cli.py
    ...
    - id: adapter_init
      ...
      file: ngram/llms/gemini_agent.py
      function: main
  ...
  docking_points:
    ...
    available:
      ...
      - id: dock_tool_result
        ...
        async_hook: optional
        needs: result verification and optional persistence to `.ngram/state`
...
agent_cli.py:select_provider → creates subprocess command
  → python -m ngram.llms.gemini_agent → gemini_agent.main() parses args
...
```
ngram/agent_cli.py
    └── spawns → ngram.llms.gemini_agent
```
...
|---------|----------|-------------|
| `google.genai` | Gemini API client, chat streaming, tool wiring | `ngram/llms/gemini_agent.py` |
| `dotenv` | `.env` file loading for credentials and overrides | `ngram/llms/gemini_agent.py` |
| `argparse` | CLI flag parsing for prompts, output format, API key, and allowed tools | `ngram/llms/gemini_agent.py` |
| `subprocess`, `glob`, `shutil`, `urllib` | Tool helpers rely on file system commands, global matching, and web fetches | `ngram/llms/gemini_agent.py` |
| `json`, `os`, `sys`, `re` | Logging stream-json, environment inspection, error handling, regex search | `ngram/llms/gemini_agent.py` |
...
|-------|----------|-------|-----------|
| `history` | `ngram/llms/gemini_agent.py` | Local (per process) | Initialized when `main` starts, extended with system prompt tokens, discarded after exit. |
| `tool_map` | `ngram/llms/gemini_agent.py` | Local helper registry | Built once after helper definitions, referenced for every tool invocation. |
| `google_search_base_url` | `ngram/llms/gemini_agent.py` | Local configuration state | Loaded once from `.env` / env vars to keep the helper deterministic. |
...
|-----------|-------|-------|
| Gemini subprocess | Single-threaded, synchronous process | The entire adapter runs on the thread started by `python -m ngram.llms.gemini_agent`; I/O is blocking but streaming flushes keep the CLI responsive. |
...
| `NGRAM_GOOGLE_SEARCH_URL` | `.env` or environment variable | `https://www.google.com/search` | Base URL for the Google search helper so tests can override it for offline runs. |
...
| File | Line | Reference |
|------|------|-----------|
| `ngram/llms/gemini_agent.py` | 2 | `# DOCS: docs/llm_agents/PATTERNS_Provider_Specific_LLM_Subprocesses.md` |

### Docs → Code

| Doc Section | Implemented In |
|-------------|----------------|
| ALGORITHM_Gemini_Stream_Flow stream loop | `ngram/llms/gemini_agent.py:main` |
| PROVIDER PATTERNS subprocess boundary | `ngram/llms/gemini_agent.py` tool helper registry |
| HEALTH coverage assertions | `ngram/llms/gemini_agent.py` (tool helper error handling) |
...
|------|---------|--------|------------|--------------|
| `ngram/llms/gemini_agent.py` | ~270L | <250L | `ngram/llms/tool_helpers.py` | Pull the repeated tool helper definitions (read, list, glob, shell, search, etc.) into a shared helper module to keep the adapter focused on wiring. |
