...
The algorithm builds a multi-section prompt by interpolating the required doc paths, state location, and VIEW table into a string template. It follows the `@ngram:id: PROMPT.INGEST.MODULE_CHAIN.FEATURE_INTEGRATION — ## Chain Hierarchy` directive by sequencing PATTERNS → BEHAVIORS → … → SYNC content and then handing the agent explicit next steps (SYNC updates, health reminder). It performs no IO except optional printing via `print_bootstrap_prompt()`.
...
Insert the absolute (or relative) `.ngram` doc paths for `PROTOCOL.md`, `PRINCIPLES.md`, and `state/SYNC_Project_State.md`. This anchors the prompt to the correct project.
...
Add steps reminding the agent to update SYNC, restate the project state, and re-run `ngram prompt --dir {target_dir}` (the same directory you bootstrapped) whenever they lose the thread. Keep the quick “start now” question immediately before the checklist so the prompt still ends with actionable guidance.
...
```
input: project_dir path (per `modules.yaml: prompt`) → render doc path strings (`PROTOCOL.md`, `PRINCIPLES.md`, `SYNC_Project_State.md`) → format work-mode guidance → build VIEW table (per `.ngram/views/VIEW_Review_Evaluate_Changes.md`) → append SYNC/health checklist → output: prompt text
```
...
|--------|--------------|-------------|
| `ngram/cli.py` | `print_bootstrap_prompt()` | human-readable prompt text |
| `ngram/prompt.py` | `generate_bootstrap_prompt()` | string used by CLI, docs, or tests |
