...
@ngram:id: CLI.PROMPT.IMPLEMENTATION
...
├── cli.py                  # Parses commands and routes `ngram prompt`
``` 
...
|------|---------|----------------------|-------|--------|
| `ngram/prompt.py` | Prompt generation helper | `generate_bootstrap_prompt`, `print_bootstrap_prompt` | ~90 | OK |
| `ngram/cli.py` | CLI routing for prompt command | `main()`, `print_bootstrap_prompt` invocation | ~400 | OK |
...
|---------|------------|---------|
| Template | `ngram/prompt.py` | Reuses a static string template with placeholders for docs and views |
| Router | `ngram/cli.py` | Maps CLI subcommands to implementation helpers |

### Anti-Patterns to Avoid

- **Heuristic prompts**: Do not hardcode agent assumptions; keep doc references canonical.
- **Prompt drift**: Keep `ngram prompt` tied to PATTERNS/BEHAVIORS docs to avoid ad hoc instructions.
...
|-------------|-----------|--------------|
| `print_bootstrap_prompt()` | `ngram/cli.py:364` | `ngram prompt` subcommand |
| `generate_bootstrap_prompt()` | `ngram/prompt.py:17` | `print_bootstrap_prompt()` |
...
flow:
  ...
  scope: `ngram/prompt.py` string assembly -> CLI output
  steps:
    - id: resolve_docs
      description: Render `.ngram` doc paths
...
- CLI prompt uses `ngram/cli.py` for the command entrypoint and relies on the doc chain (PATTERNS, BEHAVIORS, ALGORITHM, etc.) living under `docs/cli/prompt`. It also references the protocol docs to build the view table.
...
1. CLI routes `ngram prompt` to `print_bootstrap_prompt()`.
...
|--------|----------|---------|-------------|
| `prompt_docs` | config file (in `.ngram/`) | `[]` | Additional docs to include in the prompt |
| `prompt_views` | config file (in `.ngram/`) | standard VIEW table | Custom VIEW options for agents |
