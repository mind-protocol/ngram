...
for each ImportFrom node:
    if module is local (engine/, ngram/, etc.):
...
### 3. File Convention
```
tests/test_tick.py → links all test functions to engine/tick.py
```
...
```python
# DOCS: docs/engine/PATTERNS_Engine.md
...
- File paths: engine/physics/tick.py

### 3. Module Convention
```
engine/physics/*.py → docs/engine/physics/IMPLEMENTATION*.md
```
