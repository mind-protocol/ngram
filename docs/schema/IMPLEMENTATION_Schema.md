...
| `engine/graph/health/check_health.py` | CLI health check | `check_graph_health()`, `validate_node()`, `HealthReport` | ~430 | WATCH |
| `engine/graph/health/test_schema.py` | Pytest suite | `SchemaValidator`, 20+ test methods | ~880 | SPLIT |
| `engine/graph/health/schema.yaml` | Blood Ledger overlay | Character, Place, Thing, Narrative enums | ~320 | OK |
| `engine/models/base.py` | Pydantic enums | `CharacterType`, `PlaceType`, `ThingType`, etc. | ~460 | WATCH |
| `engine/models/nodes.py` | Pydantic nodes | `Character`, `Place`, `Thing`, `Narrative`, `Moment` | ~320 | OK |
| `engine/models/links.py` | Pydantic links | `CharacterNarrative`, `PlacePlace`, etc. | ~225 | OK |
...
**Why this pattern:** Base schema provides project-agnostic structure. Project schemas overlay constraints. Separation enables reuse across ngram/Blood Ledger.
...
| ngram init | — | Copies schema.yaml to .ngram/ |
...
flow:
  ...
  steps:
    - id: load_base
      ...
      file: engine/graph/health/check_health.py
      function: load_schema()
      input: BASE_SCHEMA_PATH
      output: base dict
    - id: load_project
      description: Load engine/graph/health/schema.yaml
      file: engine/graph/health/check_health.py
      function: load_schema()
  ...
  docking_points:
    available:
      ...
      - id: dock_project_loaded
        type: file
        direction: input
        file: engine/graph/health/schema.yaml
...
docs/schema/schema.yaml (authoritative)
    └── loaded by → engine/graph/health/check_health.py
    └── loaded by → engine/graph/health/test_schema.py
    └── copied to → .ngram/schema.yaml (at init)

engine/graph/health/schema.yaml (Blood Ledger)
...
| `pydantic` | Model validation | engine/models/*.py |
...
<!-- @ngram:resolved PYDANTIC_VS_SCHEMA: RESOLVED 2025-12-23 — Pydantic models now use generic schema types (Actor, Space, Thing). Character→Actor, Place→Space migration complete. No game-specific models. -->

<!-- @ngram:todo DOCS_REFERENCE_UPDATE: check_health.py:8 references obsolete docs path "docs/schema/graph-health/...". Update to current location. -->

<!-- @ngram:todo SPLIT_TEST_SCHEMA: test_schema.py at 880 lines exceeds SPLIT threshold. Should extract node tests and link tests into separate files. -->

<!-- @ngram:proposition SCHEMA_TYPING: Add Python type stubs for schema.yaml structure so IDE can provide completion when accessing loaded schema dict. -->
