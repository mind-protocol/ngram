...
STATUS: STABLE
CREATED: 2025-12-23
VERIFIED: 2025-12-23 against schema.yaml invariants section
```
...
grep -E "CREATE|SET|DELETE|MERGE" engine/graph/health/*.py
```
...
VERIFIED_AGAINST:
    impl: engine/graph/health/check_health.py @ HEAD
    test: engine/graph/health/test_schema.py @ HEAD
...
<!-- @ngram:todo V2_FULL_COVERAGE: Add explicit range validation for ALL physics fields (weight, energy, strength) on ALL node/link types. Currently only some are checked. -->

<!-- @ngram:todo V3_POLARITY_ALL_LINKS: Extend polarity range check to all link types that have polarity, not just BELIEVES. -->

<!-- @ngram:todo E2_E3_TESTS: Add tests for missing schema file and malformed YAML edge cases. Currently untested. -->

<!-- @ngram:escalation P3_BASE_SCHEMA_TEST: Need a test that validates against pure base schema without project overlay. Verifies that base schema is self-sufficient for structural validation. -->
