...
5. **Extensible through layering** — Base schema in `docs/schema/schema.yaml` defines structure. Projects overlay constraints via `engine/graph/health/schema.yaml`.
...
<!-- @ngram:resolved MODELS_ALIGNMENT: RESOLVED 2025-12-23 — Models now use generic schema types (Actor, Space, Thing). All game-specific "Character", "Place" references removed. Pydantic models match base schema. -->

<!-- @ngram:resolved CONTEXT_CLEANUP: RESOLVED 2025-12-23 — Removed game-specific Blood Ledger references from engine/ files. Schema is now project-agnostic. -->
