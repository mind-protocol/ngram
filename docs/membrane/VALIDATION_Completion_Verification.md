...
IMPL:            ngram/repair_verification.py (to be created)
```
...
- [ ] Space node missing in graph → run membrane_start("define_space")
- [ ] SYNC narrative missing → run membrane_start("update_sync")
...
- [ ] SYNC narrative not in graph → run membrane_start("update_sync")
- [ ] No update moment recorded → run membrane_start("update_sync")
...
- [ ] Create missing docs → run membrane_start("create_doc_chain")
...
- [ ] No thing node → run membrane_start("add_implementation")
```
...
| Health passes | command | `ngram doctor --module {module}` shows no critical issues |
...
- [ ] Health issues → run ngram doctor and fix critical issues
- [ ] No implementation narrative → run membrane_start("add_implementation")
```
...
- [ ] No health narrative → run membrane_start("add_health_coverage")
```
...
- [ ] No decision recorded → run membrane_start("capture_decision")
...
| No new critical issues | command | `ngram doctor` doesn't show new criticals |
...
def format_verification_feedback(results: List[VerificationResult]) -> str:
    ...
    if protocols:
        ...
        for p in protocols:
            lines.append(f"- membrane_start(\"{p}\")")
...
<!-- @ngram:todo Implement repair_verification.py with VerificationResult dataclass -->
<!-- @ngram:todo Add membrane query client for verification checks -->
<!-- @ngram:todo Integrate verification into repair_core.py spawn_repair_agent_async -->
<!-- @ngram:todo Add --continue support for agent restart -->
<!-- @ngram:todo Track retry count per issue -->
