...
Original file: SYNC_Membrane_System.md
...
- 15 skills in templates/ngram/skills/
...
**Implementation:**
- `ngram/repair_verification.py` — verification checks and feedback formatting
- `ngram/repair_core.py` — added `spawn_repair_agent_with_verification_async`
- `ngram/repair.py` — now uses verification-enabled spawn
...
- Implemented `ngram/doctor_checks_membrane.py`:
...
All protocol create steps now validate and persist to the ngram graph:
...
**v2 Opportunities:**
1. **Direct Protocol Execution** — `ngram work --protocol=update_sync` CLI command
