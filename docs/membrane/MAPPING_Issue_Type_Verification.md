...
IMPL:  ngram/repair_verification.py
...
checks:
  - name: space_exists
    ...
    expect: count >= 1
    action: "Run membrane_start('define_space')"

  - name: sync_narrative
    type: membrane
    query: { find: narrative, type: sync, in_space: "{space_id}" }
    expect: count >= 1
    action: "Run membrane_start('update_sync')"
...
feedback_template: |
  ...
  - [ ] Space node missing → membrane_start("define_space")
  - [ ] SYNC narrative missing → membrane_start("update_sync")
...
feedback_template: |
  ...
  - [ ] Narrative not updated → membrane_start("update_sync")
  - [ ] No update moment → membrane_start("update_sync")
...
checks:
  ...
  - name: sequence_links
    type: membrane
    query: { find: link, type: sequence, in_space: "{space_id}" }
    expect: count >= 4 (connecting docs)
    action: "Run membrane_start('create_doc_chain')"

feedback_template: |
  INCOMPLETE: Doc chain for {module} incomplete.
  Missing: {missing_files}
  - [ ] Create missing docs → membrane_start("create_doc_chain")
...
checks:
  ...
  - name: thing_node_exists
    ...
    expect: count >= 1
    action: "Run membrane_start('add_implementation')"
...
feedback_template: |
  ...
  - [ ] No thing node → membrane_start("add_implementation")
```
...
checks:
  ...
  - name: health_check
    type: command
    command: "ngram doctor --path {path} --quiet"
...
feedback_template: |
  ...
  - [ ] Graph: membrane_start("add_implementation")
```
...
checks:
  ...
  - name: cluster_in_graph
    ...
    action: "Run membrane_start('add_cluster')"
...
feedback_template: |
  ...
  - [ ] Graph: membrane_start("add_cluster") for new modules
```
...
checks:
  ...
  - name: health_indicators
    ...
    expect: count >= 1
    action: "Run membrane_start('add_health_coverage')"
...
feedback_template: |
  ...
  - [ ] Graph: membrane_start("add_health_coverage")
```
...
checks:
  - name: decision_recorded
    ...
    expect: count >= 1
    action: "Run membrane_start('capture_decision')"
...
feedback_template: |
  ...
  - [ ] Decision not recorded → membrane_start("capture_decision")
...
<!-- @ngram:todo Generate specs/verification.yaml from this mapping -->
<!-- @ngram:todo Create repair_verification.py implementing these checks -->
