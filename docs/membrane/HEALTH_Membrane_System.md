...
```
OBJECTIVES:      ./OBJECTIVES_Membrane_System.md
PATTERNS:        ./PATTERNS_Membrane_System.md
BEHAVIORS:       ./BEHAVIORS_Membrane_System.md
ALGORITHM:       ./ALGORITHM_Membrane_System.md
VALIDATION:      ./VALIDATION_Membrane_System.md
IMPLEMENTATION:  ./IMPLEMENTATION_Membrane_System.md
THIS:            HEALTH_Membrane_System.md
SYNC:            ./SYNC_Membrane_System.md

IMPL:            engine/connectome/runner.py
...
flows_analysis:
  - flow_id: protocol_execution
    ...
    triggers:
      - type: event
        source: tools/mcp/membrane_server.py:membrane_start
        notes: "Agent calls membrane_start with protocol name"
    frequency:
  ...
  - flow_id: answer_validation
    ...
    triggers:
      - type: event
        source: tools/mcp/membrane_server.py:membrane_continue
  ...
  - flow_id: cluster_creation
    ...
    triggers:
      - type: event
        source: engine/connectome/steps.py:execute_create
...
```yaml
status:
  stream_destination: .ngram/health/membrane_health.yaml
  result:
    representation: enum
    value: UNKNOWN
...
docks:
  input:
    id: session_start
    method: engine.connectome.runner.ConnectomeRunner.start
    location: engine/connectome/runner.py:45
  output:
    id: session_complete
    method: engine.connectome.runner.ConnectomeRunner.continue_session
    location: engine/connectome/runner.py:78
```
...
```yaml
throttling:
  trigger: membrane_continue event
...
forwarding:
  targets:
    - location: .ngram/health/membrane_health.yaml
      transport: file
      notes: Local health status file
display:
  locations:
    - surface: CLI
      location: ngram doctor --stream
...
docks:
  input:
    id: step_dispatch
    method: engine.connectome.steps.execute_step
    location: engine/connectome/steps.py:15
  output:
    id: step_complete
    method: engine.connectome.steps.execute_step
    location: engine/connectome/steps.py:45
```
...
docks:
  input:
    ...
    method: engine.connectome.steps.execute_create
    location: engine/connectome/steps.py:120
  output:
    id: graph_commit
    method: engine.physics.graph.GraphOps.create_node
    location: engine/physics/graph.py:45
```
...
ngram doctor --stream --module membrane
...
<!-- @ngram:todo V-MEM-3 (spawn depth) not yet covered - need call_protocol test -->
<!-- @ngram:todo V-MOM-1 through V-MOM-4 (moment invariants) need checkers -->
<!-- @ngram:todo V-QUERY-1 through V-QUERY-3 (query invariants) need checkers -->
<!-- @ngram:todo V-DOC-1 through V-DOC-3 (doctor invariants) need doctor integration -->

---

## MARKERS

<!-- @ngram:todo Implement session_state_checker -->
<!-- @ngram:todo Implement cluster_integrity_checker -->
<!-- @ngram:proposition Add health stream output to membrane_server.py -->
