...
@ngram:id: SKILL.ORCH.FEATURE_INTEGRATION

## Maps to VIEW
`(wrapper skill; calls the sequence below)`

## Purpose
Run the full pipeline: ingest → per-module loop → close-out, without asking the user what to do next unless truly blocked.
...
- Must load PROTOCOL and all required VIEWS referenced by downstream skills.
- Must create at least one `@ngram:TODO` per module/task discovered.
- Must enforce the pipeline order and the “never-stop” work conservation rule.
