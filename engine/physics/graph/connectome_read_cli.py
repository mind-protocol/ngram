...
Usage:
  python -m engine.physics.graph.connectome_read_cli --search "query" --threshold 0.4 --hops 2
  python -m engine.physics.graph.connectome_read_cli --cypher "MATCH (n) RETURN n"
  python -m engine.physics.graph.connectome_read_cli --nl "all links"
"""
...
from engine.physics.graph.graph_ops import GraphReadOps


def main() -> int:
    parser = argparse.ArgumentParser()
    parser.add_argument("--graph", default="seed")
    parser.add_argument("--host", default=os.getenv("NGRAM_FALKORDB_HOST", "localhost"))
    parser.add_argument("--port", type=int, default=int(os.getenv("NGRAM_FALKORDB_PORT", "6379")))
