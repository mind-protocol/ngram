...
class EnergyConservationChecker(BaseChecker):
    ...
    def check(self) -> HealthResult:
        ...
        try:
            ...
            actor_count, moment_count = self._get_counts()
            expected_max = self._calculate_expected_max(actor_count, moment_count)

            if expected_max == 0:
                return self.ok(
                    "Empty graph - no energy to check",
                    details={"total_energy": 0, "actors": 0, "moments": 0}
                )
    ...
    def _calculate_expected_max(self, actor_count: int, moment_count: int) -> float:
        """
        Calculate expected maximum energy based on graph size.

        Formula: (actors × MAX_ACTOR_ENERGY) + (moments × MAX_MOMENT_ENERGY)
        Using reasonable defaults: 10.0 per actor, 5.0 per moment
        """
        MAX_ACTOR_ENERGY = 10.0
        MAX_MOMENT_ENERGY = 5.0

        return (actor_count * MAX_ACTOR_ENERGY) + (moment_count * MAX_MOMENT_ENERGY)
