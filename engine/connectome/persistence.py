"""
Ngram Graph Persistence

Persists validated nodes and links to the ngram graph.
Enforces schema validation and connectivity constraints before write.

Usage:
    from engine.connectome.persistence import GraphPersistence, PersistenceResult
    ...
    def _persist_node(self, node: Dict[str, Any]) -> None:
        ...
        if node_type == "space":
            ...
            self.graph_ops.add_moment(
                id=node_id,
                text=node.get("prose", node.get("text", "")),
