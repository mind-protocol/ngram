# Skill: `ngram.author_protocols`
@ngram:id: SKILL.META.AUTHOR_PROTOCOLS.DESIGN_AND_STRUCTURE

## Maps to VIEW
`(meta-skill; guides creation of protocols executed by membrane tool)`

---

## Context

Protocols = YAML procedures executed by membrane. They gather context, ask questions, create graph clusters.

```
Skill (knowledge) → Protocol (procedure) → Membrane (executor) → Graph (nodes + links)
```

Protocols differ from skills:
- Skills = contextual knowledge (what to know)
- Protocols = executable steps (what to do)
...
## Purpose
Write protocols that gather context, check reality, ask rich batched questions, and create dense clusters.
...
trigger: "<gap or event>"             # what starts this protocol
...
```yaml
protocol_document:
  path: "protocols/protocol_<verb>_<object>.yaml"
...
Protocol isn't a form. It's a **guided conversation**.
...
```
Protocol run: add_health_coverage
...
**Protocol questions = guided thinking + link generation + provenance capture**
...
**Naming:**
- Protocol name: `protocol_<verb>_<object>`
- File: `protocols/protocol_<verb>_<object>.yaml`
...
```yaml
batch_questions:
  - trigger: "What gap/event starts this protocol?"
...
```yaml
protocol: protocol_<verb>_<object>
version: "1.0"
