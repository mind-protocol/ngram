# Skill: `ngram.ingest_docs_to_graph`
@ngram:id: SKILL.INGEST.DOCS_TO_GRAPH.ARCHIVE
...
Documentation files in `docs/` must be ingested into the ngram graph to enable:
...
Ingest documentation files into the ngram graph as narrative nodes, create appropriate links, and archive originals.
...
```
@ngram:skill: ingest_docs_to_graph
@ngram:trigger: DOCS_NOT_INGESTED issue
@ngram:protocol: ingest_docs
```
