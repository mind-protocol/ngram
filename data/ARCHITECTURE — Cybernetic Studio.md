...
DOCS: docs/architecture/cybernetic_studio_architecture/PATTERNS_Cybernetic_Studio_Architecture.md

STATUS: DESIGNING
LAST_UPDATED: 2025-12-20
OWNER: Nicolas + Studio (ngram platform, Blood Ledger cartridge)
SCOPE: Architecture for shipping (1) a game, (2) a dev framework, and (3) a graph-based modeling/memory/context/communication layer.

---

## 0) One-Sentence Summary

Build **two repos**: a **Studio Platform** (ngram) that provides graph physics, stimulus ingestion, places, and agent orchestration; and a **Game Cartridge** (Blood Ledger) that provides world content + player experience—both attached to the **same graph service**, where the graph holds meaning and state, while the repo holds artifacts.
...
- `blood-ledger` imports `ngram` as a library (editable install in dev; versioned package later).
...
- repo: `ngram` | `blood-ledger`
- commit_sha: `abc123`
- path: `ngram/doctor_checks.py`
...
- Updates create Moments (“SYNC updated”) and distill Narratives (“current state is…”)

SYNC rooms behave like chat rooms: they’re where state is negotiated and handed off.
...
- type: thing
  id: thing_repo_ngram_doctor_checks
  name: "ngram/doctor_checks.py"
  kind: file_ref
  repo: "ngram"
  path: "ngram/doctor_checks.py"
  head_sha: "abc123"
...
- type: narrative
  ...
  about:
    things: [thing_repo_ngram_doctor_checks]
...
- type: attached_to
  moment: mom_uuid_1
  target: thing_repo_ngram_doctor_checks
