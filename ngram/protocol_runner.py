"""
Protocol Runner — Execute protocols to create dense graph clusters.
...
Usage:
    ...
    result = runner.run("protocols/add_health_coverage.yaml", actor_id="actor_AGENT_keeper")
"""
...
class ProtocolResult:
    ...
    # Cluster metrics (added for connectivity scoring)
    connectivity_score: Optional[Any] = None
...
class ProtocolRunner:
    """
    Execute protocols to create dense graph clusters.
    ...
    def run(
        ...
        """
        Run a protocol and return the result.

        Args:
            protocol_path: Path to protocol YAML file
            actor_id: Actor running this protocol
            initial_context: Pre-populated context values

        Returns:
            ProtocolResult with created nodes/links
        """
        # Reset state
        ...
        try:
            ...
            return ProtocolResult(success=False, errors=[f"Failed to load protocol: {e}"])

        self.context['_protocol'] = protocol.get('id', protocol_path.stem)
        ...
        # Run cluster validation if enabled
        ...
        if self.validate_cluster and self.created_nodes:
            try:
                ...
                connectivity_score = validation_result['score']
    ...
    def _parse_step(self, step_def: Dict) -> ProtocolStep:
        """Parse step definition into ProtocolStep."""
    ...
    def _parse_step_v2(self, step_name: str, step_def: Dict) -> Dict:
        """Parse v2 format step (from existing protocols)."""
        return {
            'name': step_name,
    ...
    def _run_step_v2(self, step: Dict) -> Optional[str]:
        ...
        elif step_type == 'call_protocol':
            return self._run_call_protocol_step(step)
        else:
        ...
        for check in checks:
            ...
            if condition == 'ready' or self._evaluate_condition_v2(condition):
                if isinstance(action, dict):
                    if action.get('goto'):
                        return action['goto']
                    elif action.get('type') == 'call_protocol':
                        # Would call another protocol - for now, skip
                        logger.info(f"Would call protocol: {action.get('protocol')}")
                        return step.get('next')
    ...
    def _run_call_protocol_step(self, step: Dict) -> Optional[str]:
        """Execute a call_protocol step - invoke another protocol."""
        protocol_name = step.get('protocol')
        ...
        # Find protocol file
        protocol_path = Path(f"protocols/{protocol_name}.yaml")

        if protocol_path.exists():
            logger.info(f"Calling protocol: {protocol_name}")
            # Run sub-protocol with merged context
            sub_runner = ProtocolRunner(self.graph, self.answer_provider)
            sub_context = {**self.context, **expanded_inputs}
            result = sub_runner.run(protocol_path, self.context.get('_actor', 'actor_SYSTEM'), sub_context)
            ...
            if not result.success:
                for err in result.errors:
                    logger.warning(f"Sub-protocol error: {err}")
        else:
            logger.warning(f"Protocol not found: {protocol_name}")

        return step.get('next')

    def _run_step(self, step: ProtocolStep):
    ...
    def _create_completion_moment(self, on_complete: Dict, actor_id: str) -> str:
...
def run_protocol_command(
    ...
    """
    Run a protocol from CLI.

    Args:
        protocol_name: Protocol file name (without .yaml)
    ...
    if not protocol_path.exists():
        ...
        if not protocol_path.exists():
            raise FileNotFoundError(f"Protocol not found: {protocol_name}")
    ...
    return runner.run(protocol_path, actor_id)


if __name__ == "__main__":
    import argparse

    parser = argparse.ArgumentParser(description="Run a protocol")
