...
def _maybe_fix_chain_links(target_dir: Path, dry_run: bool) -> FixResult:
    ...
    for md_file in docs_dir.rglob("*.md"):
        ...
        for index, line in enumerate(lines):
            ...
            if not new_link and md_file.match("docs/protocol/HEALTH_Protocol_Verification.md"):
...
def _rewrite_impl_references(docs_dir: Path, dry_run: bool) -> FixResult:
    replacements = [
        ...
        ("`SYNC_Project_Health.md`", "`.ngram/state/SYNC_Project_Health.md`"),
        ("`SYNC_Prompt_Command_State.md`", "`.ngram/state/SYNC_Prompt_Command_State.md`"),
...
def _ensure_prompt_state_sync(docs_dir: Path, dry_run: bool) -> int:
    path = docs_dir.parent / ".ngram" / "state" / "SYNC_Prompt_Command_State.md"
    return 1 if _maybe_write_file(path, _PROMPT_SYNC_STATE, dry_run) else 0


def _ensure_ngram_config(docs_dir: Path, dry_run: bool) -> int:
    path = docs_dir.parent / ".ngram" / "config.yaml"
...
_CLI_PATTERNS = """# ngram Framework CLI — Patterns: Command Surface Overview and Scope
...
- Refactor mechanics (owned by `ngram refactor` internals).
"""


_CLI_BEHAVIORS = """# ngram Framework CLI — Behaviors: Command Surface Effects
...
_CLI_VALIDATION = """# ngram Framework CLI — Validation: Command Invariants
...
_CLI_HEALTH = """# ngram Framework CLI — Health: Verification Checklist
...
_CLI_SYNC = """# ngram Framework CLI — Sync: Current State
