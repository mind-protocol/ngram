...
DOCS: .ngram/skills/SKILL_Test_Integrate_And_Gate_Completion.md
"""
...
def parse_invariants(validation_file: Path) -> List[Invariant]:
    ...
    for i, line in enumerate(lines, 1):
        ...
        # Pattern 1: ### @ngram:id: V-XXX
        id_match = re.search(r'@ngram:id:\s*(V-[\w-]+)', line)
        # Pattern 2: ### @ngram:area:module:invariant:V-XXX
        invariant_match = re.search(r'@ngram:[\w:]+:invariant:(V-[\w-]+)', line)
        ...
        # Also look for @ngram:...:test: markers that link to invariants
        test_marker = re.search(r'@ngram:[\w:]+:test:([\w_]+)', line)
...
def parse_test_mappings(test_file: Path) -> List[TestMapping]:
    ...
    - @ngram:area:module:validates:V-ID
    """
    mappings = []
    ...
    for i, line in enumerate(lines, 1):
        ...
        # Look for @ngram:...:validates:V-ID patterns
        ngram_validates = re.search(r'@ngram:[\w:]+:validates:(V-[\w-]+)', line)
        if ngram_validates and current_function:
            inv_id = ngram_validates.group(1)
...
def doctor_check_completion_gate(target_dir: Path, config: DoctorConfig) -> List[DoctorIssue]:
    ...
    Reads .ngram/completion/*.yaml and reports incomplete modules.
    ...
    completion_dir = target_dir / ".ngram" / "completion"
...
def generate_completion_gate(
    ...
    Creates .ngram/completion/{module}_gate.yaml
    """
