...
# DOCS: docs/protocol/doctor/IMPLEMENTATION_Project_Health_Doctor.md
...
def _find_associated_sync_file(doc_path: Path, target_dir: Path) -> Optional[Path]:
    ...
    fallback = project_root / ".ngram" / "state" / "SYNC_Project_State.md"
...
def doctor_check_prompt_checklist(target_dir: Path, config: DoctorConfig) -> List[DoctorIssue]:
    ...
    if missing:
        issues.append(DoctorIssue(
            ...
            details={"missing": missing},
            suggestion="Add the checklist reminding agents to update SYNC and rerun `ngram prompt --dir`"
        ))
