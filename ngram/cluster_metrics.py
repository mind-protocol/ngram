"""
Cluster Metrics — Connectivity scoring, target validation, and link suggestions.

Implements three capabilities from SKILL_Add_Cluster_Dynamic_Creation:
1. Connectivity Scoring - links_per_node, external_ratio, orphan_score
...
class ConnectivityScore:
    """Connectivity metrics for a cluster."""
    total_nodes: int = 0
    total_links: int = 0
    ...
    def calculate_verdict(self) -> str:
        ...
        if self.links_per_node < self.LINKS_MIN:
            return "SPARSE - Below minimum connectivity"
...
VALID_TARGETS = {
    'narrative.health': {
        ...
        'attached_to': {'source': ['thing.dock'], 'exactly': 2},  # Reversed - docks attach to health
    },
    ...
    'narrative.algorithm': {
        'implements': {'target': ['narrative.pattern'], 'min': 1, 'max': None},
        'enables': {'target': ['narrative.behavior'], 'min': 0, 'max': None},
    },
    'narrative.behavior': {
        'achieves': {'target': ['narrative.objective'], 'min': 1, 'max': None},
        'relates': {'target': ['narrative.pattern'], 'min': 0, 'max': None},
    },
    'narrative.pattern': {
        'supports': {'target': ['narrative.objective'], 'min': 1, 'max': None},
        'relates': {'target': ['narrative.pattern'], 'min': 0, 'max': None},
    },
    'narrative.objective': {
        'supports': {'source': ['narrative.objective'], 'min': 0, 'max': None},  # Secondary supports primary
        'bounds': {'source': ['narrative.objective'], 'min': 0, 'max': None},  # Non-objectives
    },
    'thing.dock': {
        'attached_to': {'target': ['narrative.health'], 'exactly': 1},
...
REQUIRED_FIELDS = {
    ...
    'narrative.validation': ['name', 'invariant'],
    'narrative.algorithm': ['name', 'steps'],
    'narrative.behavior': ['name', 'given', 'when', 'then'],
    'narrative.pattern': ['name', 'content'],
    'narrative.objective': ['name', 'content'],
...
class ClusterMetrics:
    """
    Calculate cluster connectivity and provide link suggestions.
    """
    ...
    def score_cluster(
        ...
        """
        Calculate connectivity score for a cluster.
        ...
        Returns:
            ConnectivityScore with metrics and verdict
        """
    ...
    def format_score(self, score: ConnectivityScore) -> str:
        """Format connectivity score for display."""
        lines = [
            "CONNECTIVITY SCORE",
            "=" * 40,
    ...
    def validate_targets(
        ...
        """
        Validate that links have valid target types.
        ...
        for link in links_created:
            ...
            # Validate target type if rule exists
    ...
    def _get_node_type(self, node_id: str) -> str:
        ...
        if node_id.startswith('narrative_'):
            parts = node_id.split('_')
            if len(parts) >= 2:
                return f"narrative.{parts[1].lower()}"
        elif node_id.startswith('thing_'):
            parts = node_id.split('_')
            if len(parts) >= 2:
                return f"thing.{parts[1].lower()}"
        elif node_id.startswith('space_'):
            return 'space.module'
        elif node_id.startswith('moment_'):
            return 'moment.event'
...
class ClusterValidator:
    ...
    def validate_cluster(
        ...
        valid = (
            ...
            score.links_per_node >= ConnectivityScore.LINKS_MIN
        )
