...
def get_issue_explanation(issue_type: str) -> Dict[str, str]:
    ...
    explanations = {
        ...
        "INCOMPLETE_CHAIN": {
            ...
            "action": "Create the missing doc types using templates from .ngram/templates/.",
        },
        "NO_DOCS_REF": {
            "risk": "Without a DOCS: reference, the bidirectional link is broken. Agents reading code can't find the design docs, and `ngram context` won't work.",
...
def generate_health_markdown(results: Dict[str, Any], github_issues: List = None) -> str:
    ...
    lines.append("UPDATED_BY: ngram doctor")
    score = results['score']
    if score >= 80:
        status = "HEALTHY"
    elif score >= 50:
    ...
    lines.append("---")
    lines.append("")
    lines.append("*Generated by `ngram doctor`*")
