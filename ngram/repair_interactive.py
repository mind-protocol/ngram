"""
Interactive repair functions for CLI.

Extracted from repair.py to reduce monolith size.
Contains:
- Manager agent functionality (input listener, spawn, check)
- Interactive conflict resolution (ARBITRAGE)
...
def resolve_arbitrage_interactive(issue) -> List[ArbitrageDecision]:
    """Interactively resolve ARBITRAGE conflicts with user input.

    Args:
        issue: DoctorIssue with issue_type == "ARBITRAGE"

    Returns:
        List of ArbitrageDecision with user's choices
    """
    decisions = []
...
def reset_manager_state():
    """Reset the global manager state. Call before starting a repair session."""
