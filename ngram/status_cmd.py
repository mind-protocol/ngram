...
def format_global_status(statuses: List[ModuleStatus], all_issues: List[HealthIssue]) -> str:
    ...
    lines.append(f"{C.BOLD}{C.BRIGHT_WHITE}  NGRAM PROJECT STATUS{C.RESET}")
    lines.append(f"{C.BOLD}{C.BRIGHT_WHITE}{'═' * 78}{C.RESET}")
    ...
    lines.append("")
    lines.append(f"  {C.DIM}{'─' * 76}{C.RESET}")
    lines.append(f"  {C.DIM}Run 'ngram status <module>' for detailed module view{C.RESET}")
    lines.append(f"  {C.DIM}Run 'ngram status <module> -v' for verbose output with issue details{C.RESET}")
    lines.append(f"  {C.DIM}Run 'ngram doctor' for full health report{C.RESET}")
    lines.append("")
