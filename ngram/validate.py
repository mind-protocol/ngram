...
"""
Validation command for ngram CLI.
...
def check_protocol_installed(target_dir: Path) -> ValidationResult:
    """V6 (partial): Check that .ngram/ exists."""
    ...
    if not protocol_dir.exists():
        return ValidationResult(
            ...
            details=[f"Missing: {protocol_dir}/", "Run: ngram init"]
        )
...
def check_project_sync_exists(target_dir: Path) -> ValidationResult:
    ...
    sync_path = target_dir / ".ngram" / "state" / "SYNC_Project_State.md"
...
def generate_fix_prompt(target_dir: Path, results: List[ValidationResult]) -> str:
    ...
    for result in failed_results:
        ...
        if result.check_id == "V6" and "SYNC" in result.name:
            ...
            prompt_parts.append("Edit `.ngram/state/SYNC_Project_State.md` and fill in:\n")
            ...
            prompt_parts.append("- Template: `.ngram/templates/SYNC_TEMPLATE.md`\n")
            prompt_parts.append("- Protocol: `.ngram/PROTOCOL.md` → \"SYNC files track current state\"\n\n")

        elif result.check_id == "V6" and "installed" in result.name.lower():
            prompt_parts.append("### What's Wrong\n")
            prompt_parts.append("The ngram is not installed or is incomplete.\n\n")
            for detail in result.details:
                prompt_parts.append(f"- {detail}\n")

            prompt_parts.append("\n### How to Fix\n")
            prompt_parts.append("Run: `ngram init --force`\n\n")
            ...
            prompt_parts.append("\n### How to Fix\n")
            prompt_parts.append("Re-initialize the protocol: `ngram init --force`\n\n")
            ...
            prompt_parts.append("### Reference\n")
            prompt_parts.append("- VIEW: `.ngram/views/VIEW_Document_Create_Module_Documentation.md`\n")
            prompt_parts.append("- Templates: `.ngram/templates/` (one for each doc type)\n\n")
            ...
            prompt_parts.append("- Automatic discovery by tools (`ngram context`)\n")
            ...
            prompt_parts.append("- The `ngram context` command returns incomplete results\n")
            ...
            prompt_parts.append("- `ngram context` knows which docs to load\n")
    ...
    prompt_parts.append("3. **Run validation again** after fixes: `ngram validate`\n")
