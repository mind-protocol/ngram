"""
Validation command for Context Protocol CLI.
...
def check_protocol_installed(target_dir: Path) -> ValidationResult:
    """V6 (partial): Check that .context-protocol/ exists."""
    ...
    if not protocol_dir.exists():
        return ValidationResult(
            ...
            details=[f"Missing: {protocol_dir}/", "Run: context-protocol init"]
        )
...
def check_project_sync_exists(target_dir: Path) -> ValidationResult:
    ...
    sync_path = target_dir / ".context-protocol" / "state" / "SYNC_Project_State.md"
...
def generate_fix_prompt(target_dir: Path, results: List[ValidationResult]) -> str:
    ...
    for result in failed_results:
        ...
        if result.check_id == "V6" and "SYNC" in result.name:
            ...
            prompt_parts.append("Edit `.context-protocol/state/SYNC_Project_State.md` and fill in:\n")
            ...
            prompt_parts.append("- Template: `.context-protocol/templates/SYNC_TEMPLATE.md`\n")
            prompt_parts.append("- Protocol: `.context-protocol/PROTOCOL.md` → \"SYNC files track current state\"\n\n")

        elif result.check_id == "V6" and "installed" in result.name.lower():
            prompt_parts.append("### What's Wrong\n")
            prompt_parts.append("The Context Protocol is not installed or is incomplete.\n\n")
            for detail in result.details:
                prompt_parts.append(f"- {detail}\n")

            prompt_parts.append("\n### How to Fix\n")
            prompt_parts.append("Run: `context-protocol init --force`\n\n")
            ...
            prompt_parts.append("\n### How to Fix\n")
            prompt_parts.append("Re-initialize the protocol: `context-protocol init --force`\n\n")
            ...
            prompt_parts.append("- Templates: `.context-protocol/templates/PATTERNS_TEMPLATE.md`, `SYNC_TEMPLATE.md`\n\n")
            ...
            prompt_parts.append("### Reference\n")
            prompt_parts.append("- VIEW: `.context-protocol/views/VIEW_Document_Create_Module_Documentation.md`\n")
            prompt_parts.append("- Templates: `.context-protocol/templates/` (one for each doc type)\n\n")
            ...
            prompt_parts.append("- Automatic discovery by tools (`context-protocol context`)\n")
            ...
            prompt_parts.append("- The `context-protocol context` command returns incomplete results\n")
            ...
            prompt_parts.append("- `context-protocol context` knows which docs to load\n")
    ...
    prompt_parts.append("3. **Run validation again** after fixes: `context-protocol validate`\n")
